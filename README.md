...
#### The working practice used in a Client-server network would be on how you will use this in a day to day basis. The Client-server network would work mostly in a workplace and not at home. This is because you might not have the space for theses servers to be setup within your home as huge number of clients might be attempting to access the server at the same time. The clients would connect from their own personal computers to then connect to the server to access it.
...
### 4. Cluster
#### A cluster network is a set of loosely or tightly connected computers that work together so that, they can be viewed as a single system. Unlike grid network, cluster network has each node set to perform the same task, controlled and scheduled by software.
...
### 5. Centralised
#### A centralised network is a type of network where all users are connected to a central server, which is the acting agent for all communications between. This central server will be then storing both the communications and the user account information. 
#### The benefits of using a centralised network is that there does not need to be many servers that need configurations, it has better management of servers that reduces cost and effort in maintaining it and there is a centralised approach to monitoring, managing, and reducing cost and effort of operations.
#### The resources that are required is a set of computers and a single server, this server is then connected to each of the computers via cabling, this then allows the connection between each of the computers and to the server.
#### The way each of the systems communicate is through the single server that each of the systems are connected to, this server is connected to every system which then allows each of the users to communicate with each other.
#### This is most likely used for small to medium sized businesses that would want to setup a centralised server for projects that the company may do or for having it around the whole company so that there will be less maintenance.
...
#### The network that I had created and designed has successfully met the requirements and specifications to a great standard. Many positive choices were made when creating the system, such as the star topology that was used so that the data can be smoothly sent to the destination for both networks and not to other devices on the network within the WAN. The use of the copper straight-through ethernet connection worked as intended, not as fast as expected but since it is a small sized network, it is cost effective. During the testing phase, the network performed exactly is intended and as expected no errors or bugs were found. Also, when adding in the DNS into the network, it was time consuming but once it was set-up it worked perfectly and having each of the PCs ping back exactly as it should and found out that there were no issues with pinging other users via their domain name. Improvements that could be considered to the network would be changing to other topologies that would fit the requirements much better, as other network topologies have more ways to backup, for example if the switch on the star network fails then everything connected to it cannot communicate with other devices, but if created in a mesh network, one system may fail but the others will still be able to communicate with each other. Overall, the network has met all the targets and does function well for a home or small business, but improvements to the structure of the network would prove for it to function more effectively, as well as ensuring security and better reliability.
